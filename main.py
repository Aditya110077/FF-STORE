...
REGIONS = {
    "region_ind": "India",
    "region_ban": "Bangladesh",
    "region_pak": "Pakistan",
    "region_ind2": "Indonesia",
...
def db_init():
    ...
    c.execute("""CREATE TABLE IF NOT EXISTS utrs (
        utr TEXT PRIMARY KEY, user_id INTEGER, order_id INTEGER, created_at TEXT)""")
    # Migration: purane DB me naye columns
    c.execute("PRAGMA table_info(orders)")
    cols = [r[1] for r in c.fetchall()]
    if "ff_uid" not in cols:
        c.execute("ALTER TABLE orders ADD COLUMN ff_uid TEXT")
...
# ================= STYLISH QR =================
def make_stylish_qr_url(upi_link):
    """Stylish QR - gradient color with custom design"""
    encoded = urllib.parse.quote(upi_link, safe='')
    # qrserver.com with custom colors - gold on dark
    return (
        f"https://api.qrserver.com/v1/create-qr-code/"
        f"?size=600x600&data={encoded}&color=000000&bgcolor=FFD700&qzone=2&format=png"
    )
...
def region_kb():
    ...
    for key, name in REGIONS.items():
        kb.append([InlineKeyboardButton(f"{E_GLOBE}  {name}", callback_data=key)])
    kb.append([InlineKeyboardButton(f"{E_BACK}  Back", callback_data="buylike")])
...
async def cmd_myorders(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    for r in rows:
        oid, _, pkey, pname, amt, days, utr, status, cat, vat, ff_uid, region = (r + ("",""))[:12]
...
# ================= MAIN TEXT HANDLER (UID, REGION, UTR) =================
    ...
    if ctx.user_data.get("awaiting_utr"):
        ...
        if is_fake:
            await update.message.reply_text(
                ...
                f"Reason: {reason}\n\n"
                f"Kripya asli transaction ID bhejo."
            )
            try:
                await ctx.bot.send_message(
                    OWNER_ID,
        ...
        if recent >= 3:
            await update.message.reply_text(
                f"{E_CROSS}  Aapne 24 ghante me 3 orders kar liye hain.\n\n"
        ...
        if not pending_key or not ff_uid or not region:
            await update.message.reply_text(f"{E_CROSS}  Data missing. /buy se dobara start karo.")
            return
...
async def cb_region(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    q = update.callback_query; await q.answer()
    region_key = q.data
    region = REGIONS.get(region_key, "Other")
    ...
    caption = (
        ...
        f"Payment ke baad 12-digit UTR / Transaction ID yaha type karke bhejo."
    )
