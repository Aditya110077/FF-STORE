...
def fmt_order(r):
    """Order tuple ko 12-length me pad kare"""
...
def make_stylish_qr_url(upi_link):
    """Devil style QR - dark red bg + black QR + gold border feel"""
...
async def cmd_profile(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    row = db_get_user(uid)
    if not row:
        await update.message.reply_text(f"{E_CROSS}  User not found.", parse_mode="Markdown"); return
...
async def cmd_broadcast(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    for uid in users:
        try:
            ...
            fail += 1
    await status.edit_text(f"{E_CHECK}  Sent: {sent}\n{E_CROSS}  Failed: {fail}", parse_mode="Markdown")

async def cmd_pending(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    if update.effective_user.id != OWNER_ID: return
    rows = db_pending_orders()
    if not rows:
        await update.message.reply_text(f"{E_CHECK}  Koi pending order nahi.", parse_mode="Markdown"); return
...
async def cmd_todayorders(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    today = datetime.now().strftime("%Y-%m-%d")
    if not rows:
        await update.message.reply_text(f"Aaj ({today}) koi order nahi.", parse_mode="Markdown"); return
...
async def cmd_search(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if not order:
        con.close()
        await update.message.reply_text(f"{E_CROSS}  UTR `{utr}` nahi mila.", parse_mode="Markdown"); return
    order = fmt_order(order)
    oid, uid, pkey, pname, amt, days, utr, status, cat, vat, ff_uid, region = order
    c.execute("SELECT * FROM users WHERE user_id=?", (uid,))
    user = c.fetchone(); con.close()
    text = (
        f"{E_CHART}  *SEARCH RESULT*\n\n"
        f"🆔  Order: *#{oid}*\n"
        f"👤  User ID: `{uid}`\n"
        f"📛  Name: {user[2] if user else 'N/A'}\n"
        f"{E_ID}  FF UID: `{ff_uid}`\n"
        f"{E_GLOBE}  Region: {region}\n"
        f"{E_BOX}  Plan: {pname}\n"
        f"{E_MONEY}  Amount: Rs.{amt}\n"
        f"🔖  UTR: `{utr}`\n"
...
async def cmd_reject(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    uid = order[1]
    await update.message.reply_text(f"{E_CROSS}  Order #{oid} rejected.", parse_mode="Markdown")
    try:
        await ctx.bot.send_message(uid,
...
async def cmd_verify(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    success_msg = (
        ...
        f"━━━━━━━━━━━━━━━━━━━━━━\n"
        f"{E_GAME}  *FF UID:*  `{ff_uid}`\n"
        f"{E_GLOBE}  *Region:*  {region}\n"
        f"{E_BOX}  *Plan:*  {plan['name']}\n"
        f"{E_HOUR}  *Days:*  {plan['days']} days\n"
        f"📅  *Valid Until:*  {until[:10]}\n"
        f"{E_MONEY}  *Amount Paid:*  Rs.{plan['price']}\n"
        f"🔖  *UTR:*  `{utr}`\n"
        f"🆔  *Order ID:*  #{oid if oid else 'N/A'}\n"
        f"━━━━━━━━━━━━━━━━━━━━━━\n\n"
        f"{E_HEART}  *200 Likes Daily Given*\n"
        f"{E_ROCKET}  *Status:*  ACTIVE  🟢\n"
        f"{E_HOUR}  *Delivery:*  Within 5 minutes\n\n"
        f"━━━━━━━━━━━━━━━━━━━━━━\n"
        f"💖  *Thank you for purchasing!*\n"
        f"{E_PHONE}  *Support:*  {OWNER_USERNAME}"
    )
    try:
...
async def handle_text_input(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if ctx.user_data.get("awaiting_uid"):
        ...
        await update.message.reply_text(
            f"╔══════════════════════╗\n"
            f"   {E_CHECK}  *UID SAVED*  {E_CHECK}\n"
            f"╚══════════════════════╝\n\n"
            f"{E_ID}  *FF UID:*  `{text}`\n"
            f"{E_BOX}  *Plan:*  {plan.get('name', '')}\n\n"
            f"━━━━━━━━━━━━━━━━━━━━━━\n"
            f"{E_GLOBE}  *STEP 2 - SELECT REGION*\n"
            f"━━━━━━━━━━━━━━━━━━━━━━\n\n"
    ...
    if ctx.user_data.get("awaiting_utr"):
        ...
        if db_utr_used(text):
            await update.message.reply_text(
                f"{E_CROSS}  *Ye UTR pehle use ho chuka hai!*\n\nNaya UTR bhejo ya support se contact karo.",
                parse_mode="Markdown"
            )
            return
        ...
        owner_msg = (
            f"╔══════════════════════╗\n"
            f"   {E_JOIN}  *NEW ORDER*  {E_JOIN}\n"
            f"╚══════════════════════╝\n\n"
            f"🆔  Order:  *#{oid}*\n"
            f"{E_USER}  User:  [{u.first_name}](tg://user?id={u.id})\n"
            f"🆔  ID:  `{u.id}`\n"
            f"{E_GAME}  FF UID:  `{ff_uid}`\n"
            f"{E_GLOBE}  Region:  *{region}*\n"
            f"{E_BOX}  Plan:  {plan['name']}\n"
            f"{E_MONEY}  Amount:  Rs.{plan['price']}\n"
            f"🔖  UTR:  `{text}`\n\n"
            f"━━━━━━━━━━━━━━━━━━━━━━\n"
            f"{E_CHECK}  `/verify {u.id} {pending_key}`\n"
            f"{E_CROSS}  `/reject {oid}`"
        )
        try:
        ...
        await update.message.reply_text(
            f"╔══════════════════════╗\n"
            f"   {E_CHECK}  *ORDER PLACED*  {E_CHECK}\n"
            f"╚══════════════════════╝\n\n"
            f"🆔  Order ID:  *#{oid}*\n"
            f"{E_GAME}  FF UID:  `{ff_uid}`\n"
            f"{E_GLOBE}  Region:  *{region}*\n"
...
async def cb_select_plan(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    text = (
        f"╔══════════════════════╗\n"
        f"   {E_ID}  *STEP 1 - ENTER UID*  {E_ID}\n"
        f"╚══════════════════════╝\n\n"
        f"{E_BOX}  *Plan:*  {plan['name']}\n"
        f"{E_MONEY}  *Price:*  Rs.{plan['price']}\n"
...
async def cb_region(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    caption = (
        f"╔══════════════════════╗\n"
        f"   {E_DEVIL}  *ORDER SUMMARY*  {E_DEVIL}\n"
        f"╚══════════════════════╝\n\n"
        f"{E_GAME}  *FF UID:*  `{ff_uid}`\n"
        f"{E_GLOBE}  *Region:*  {region}\n"
        f"{E_BOX}  *Plan:*  {plan['name']}\n"
        ...
        f"💳  *UPI:*  `{UPI_ID}`\n"
        f"━━━━━━━━━━━━━━━━━━━━━━\n\n"
        f"{E_DEVIL}  *QR scan karke Rs.{plan['price']} pay karo.*\n\n"
        f"{E_CHECK}  *Payment ke baad 12-digit UTR yaha type karke bhejo.*"
    )
...
async def cb_back_home(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    text = (
        f"╔══════════════════════╗\n"
        f"   {E_GAME}  *FF AUTOLIKE STORE*  {E_GAME}\n"
        f"╚══════════════════════╝\n\n"
        f"{E_HEART}  200 Likes Daily Given\n"
        f"{E_BOLT}  Instant Auto-Verify\n\n"
        f"👇  *Niche se option choose karo*"
...
async def cb_admin_home(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if q.from_user.id != OWNER_ID:
        await q.answer(f"{E_CROSS}  Not owner", show_alert=True); return
    await q.answer()
    await smart_edit(q, f"╔══════════════════════╗\n   {E_CROWN}  *ADMIN PANEL*  {E_CROWN}\n╚══════════════════════╝\n\n👇  Choose:", admin_kb())
